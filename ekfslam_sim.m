...
while iwp ~= 0
    ...
    if dtsum >= DT_OBSERVE
        ...
        if SWITCH_USE_IEKF == 1
            ...
            [x,P]= update_ekf(x,P,zf,RE,idf, SWITCH_BATCH_UPDATE); 
        end
    ...
    set(h.xt, 'xdata', xt(1,:), 'ydata', xt(2,:))%������ɫ����-ʵ��λ��
    set(h.xv, 'xdata', xv(1,:), 'ydata', xv(2,:))%������ɫ����-Ԥ��λ��
    set(h.xf, 'xdata', x(4:2:end), 'ydata', x(5:2:end))%����Ԥ���·��λ��
    ptmp= make_covariance_ellipses(x(1:3),P(1:3,1:3));%����Ԥ��λ�ò�ȷ������Բ�Ļ��Ƶ�
    pcov(:,1:size(ptmp,2))= ptmp;
    if dtsum==0
        set(h.cov, 'xdata', pcov(1,:), 'ydata', pcov(2,:)) %����Ԥ��λ�ò�ȷ����Բ
        pcount= pcount+1;
        if pcount == 15
            set(h.pth, 'xdata', data.path(1,1:data.i), 'ydata', data.path(2,1:data.i))%����Ԥ��·��    
            pcount=0;
        end
        if ~isempty(z)
            plines= make_laser_lines (z,x(1:3));%���㼤��ɨ����
            set(h.obs, 'xdata', plines(1,:), 'ydata', plines(2,:))%���Ƽ���ɨ����
            pcov= make_covariance_ellipses(x,P);%���²�ȷ����
        end
    end
    drawnow
end
